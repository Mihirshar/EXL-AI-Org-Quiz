...
import { MONTH_MARKERS } from '@/lib/gameData';
...
export default function MonthTimeline({ currentLevel, completedLevels, isComplete = false }: MonthTimelineProps) {
  ...
  const decisionMonths = MONTH_MARKERS; // [1, 4, 7, 10, 12]
  ...
  const getMonthStatus = (month: number): 'completed' | 'current' | 'pending' => {
    ...
    if (decisionIndex === -1) {
      // Non-decision month - check if we've passed it
  ...
  const getProgressWidth = () => {
    ...
    // Progress line should reach the current decision point
    const currentMonth = decisionMonths[currentLevel];
    if (!currentMonth) return 0;
    
    return ((currentMonth - 1) / 11) * 100;
  };
